...
#                self.mqtt.mqtt_client.publish(self.config['mode_state_topic'], "off" if self.attributes['authorization'] == "STOP" else "heat", qos=0)
        ...
        #print("Electric created / updated : ", self.name, self.id, self.current_position)       

    async def put_temperature(tydom_client, device_id, electric_id, set_setpoint):
        print(lectric_id, 'set_setpoint', set_setpoint)
        if not (set_setpoint == ''):
            await tydom_client.put_devices_data(device_id, electric_id, 'setpoint', set_setpoint) 		

    async def put_hvacMode(tydom_client, device_id, electric_id, set_hvacMode):
        print(electric_id, 'set_hvacMode', set_hvacMode)
        if set_hvacMode == 'off':
            await tydom_client.put_devices_data(device_id, electric_id, 'thermicLevel', 'STOP')
        else:
            await tydom_client.put_devices_data(device_id, electric_id, 'thermicLevel', 'COMFORT')
            await tydom_client.put_devices_data(device_id, electric_id, 'setpoint', '10')

    async def put_thermicLevel(tydom_client, device_id, electric_id, set_thermicLevel):
        print(electric_id, 'thermicLevel', set_thermicLevel)
        if not (set_thermicLevel == ''):
            await tydom_client.put_devices_data(device_id, electric_id, 'thermicLevel', set_thermicLevel)
